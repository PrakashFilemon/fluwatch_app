...
import {
  adminPengguna, adminUbahPengguna, adminHapusPengguna,
  adminLaporan,  adminHapusLaporan,
  masukAPI,
} from "../services/api";
  ...
  return (
    <div className="flex items-center justify-between pt-2">
      <p className="text-sm" style={{ color: "#94a3b8" }}>
        Menampilkan <span style={{ color: "#e2e8f0", fontWeight: 600 }}>
          {Math.min((halaman - 1) * PER_HAL + 1, total)}–{Math.min(halaman * PER_HAL, total)}
        </span> dari <span style={{ color: "#e2e8f0", fontWeight: 600 }}>{total}</span> data
      </p>
  ...
  return (
    <tr>
      <td colSpan={99} className="py-16 text-center">
        ...
        <p className="text-sm font-medium" style={{ color: "#64748b" }}>{teks}</p>
      </td>
    </tr>
  );
}
    ...
    <tr key={i} style={{ borderTop: "1px solid #1e293b" }}>
      {Array.from({ length: cols }).map((_, j) => (
        <td key={j} className="px-4 py-3.5">
          <div className="h-4 rounded animate-pulse" style={{ background: "#1e293b", width: j === 0 ? "70%" : "50%" }} />
        </td>
      ))}
    </tr>
  ));
    ...
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="rounded-2xl p-8"
          style={{ background: "#0d1627", border: "1px solid #1e3a5f", boxShadow: "0 25px 50px rgba(0,0,0,0.6)" }}>
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold" style={{ color: "#f1f5f9" }}>FluWatch Admin</h1>
            <p className="text-sm mt-1" style={{ color: "#64748b" }}>Masuk dengan akun administrator</p>
          </div>
          ...
          <form onSubmit={kirim} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                ...
                style={{ background: "#111827", border: "1px solid #1e3a5f", color: "#f1f5f9" }}
...
function TabPengguna({ adminId }) {
  ...
  const muat = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminPengguna({ halaman, per_halaman: PER_HAL, cari });
      setData(res);
    } catch (err) {
      toast.error(err.message);
    } finally {
  ...
  const hapus = async (id, username) => {
    if (!confirm(`Yakin hapus pengguna "${username}"? Tindakan ini tidak bisa dibatalkan.`)) return;
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3 items-center">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            style={{ background: "#111827", border: "1px solid #1e293b", color: "#e2e8f0" }}
            onFocus={e => e.target.style.borderColor = "#3b82f6"}
      ...
      <div className="rounded-2xl overflow-hidden"
        style={{ border: "1px solid #1e293b", boxShadow: "0 4px 24px rgba(0,0,0,0.4)" }}>
        <table className="w-full">
          <thead>
            <tr style={{ background: "linear-gradient(90deg,#0f172a,#0d1627)" }}>
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0"
                      style={{ background: p.role === "admin" ? "#1e3a5f" : "#1e293b", color: p.role === "admin" ? "#93c5fd" : "#94a3b8" }}>
                      {p.username[0].toUpperCase()}
                    </div>
                    <div>
                      <p className="text-sm font-semibold" style={{ color: "#e2e8f0" }}>{p.username}</p>
                      {p.id === adminId && (
                        <p className="text-xs" style={{ color: "#f97316" }}>Anda</p>
                      )}
                    </div>
                  </div>
                </td>
                <td className="px-5 py-4 text-sm" style={{ color: "#94a3b8" }}>{p.email}</td>
                <td className="px-5 py-4">
                  <Badge
                ...
                <td className="px-5 py-4">
                  {p.id !== adminId ? (
                    <div className="flex gap-2 flex-wrap">
                      <Btn onClick={() => ubahRole(p.id, p.role === "admin" ? "pengguna" : "admin")}
                        variant={p.role === "admin" ? "warning" : "default"}>
                        {p.role === "admin" ? "Turunkan" : "Jadikan Admin"}
                      </Btn>
                      <Btn onClick={() => toggleAktif(p.id, p.is_active)}
                        variant={p.is_active ? "warning" : "success"}>
                        {p.is_active ? "Nonaktifkan" : "Aktifkan"}
                      </Btn>
                      <Btn onClick={() => hapus(p.id, p.username)} variant="danger">
                        Hapus
                      </Btn>
                    </div>
                  ) : (
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#0d1627", border: "1px solid #1e3a5f", boxShadow: "0 32px 64px rgba(0,0,0,0.7)", maxHeight: "90vh", overflowY: "auto" }}
      >
        {/* Header modal */}
        <div className="flex items-start justify-between p-6"
          style={{ borderBottom: "1px solid #1e293b", background: "linear-gradient(135deg,#0f172a,#0d1627)" }}>
          <div>
        ...
        <div className="p-6 space-y-6">
            ...
            {[
              {
                ...
                bg: warna.bg, text: warna.text,
              },
              {
                label: "Keparahan",
                value: `${laporan.tingkat_keparahan}/10`,
                sub: laporan.tingkat_keparahan >= 7 ? "Berat" : laporan.tingkat_keparahan >= 4 ? "Sedang" : "Ringan",
              ...
              },
              {
                ...
                sub: laporan.sudah_vaksin === true ? "Sudah vaksin" : laporan.sudah_vaksin === false ? "Belum vaksin" : "Tidak diketahui",
                bg: "#0f172a", text: "#e2e8f0",
              },
            ].map(s => (
              <div key={s.label} className="rounded-xl p-4" style={{ background: s.bg, border: "1px solid #1e293b" }}>
                <p className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "#64748b" }}>{s.label}</p>
                <p className="text-xl font-bold capitalize" style={{ color: s.text }}>{s.value}</p>
            ...
            <div className="flex gap-1">
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="flex-1 h-3 rounded"
                  style={{ background: i < laporan.tingkat_keparahan ? warna.text : "#1e293b", transition: "background 0.2s" }} />
              ))}
            </div>
            <div className="flex justify-between mt-1">
              <span className="text-xs" style={{ color: "#334155" }}>Ringan</span>
              <span className="text-xs" style={{ color: "#334155" }}>Berat</span>
            </div>
          </div>
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "#64748b" }}>
              Gejala yang Dilaporkan ({gejalaDimiliki.length} dari {SEMUA_GEJALA.length})
            </p>
            <div className="grid grid-cols-2 gap-2">
              {gejalaDimiliki.map(g => (
                <div key={g.key} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl"
                  style={{ background: "#052e16", border: "1px solid #166534" }}>
                  <span className="text-base">{g.icon}</span>
                  <span className="text-sm font-semibold" style={{ color: "#86efac" }}>{g.label}</span>
              ...
              {gejalaTidak.map(g => (
                <div key={g.key} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl"
                  ...
                  <span className="text-sm" style={{ color: "#334155" }}>{g.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: "#0f172a", border: "1px solid #1e293b" }}>
              <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "#64748b" }}>Lokasi GPS</p>
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: "#475569" }}>Latitude</span>
                  <span className="text-sm font-mono font-semibold" style={{ color: "#e2e8f0" }}>
                    {typeof laporan.lat === "number" ? laporan.lat.toFixed(6) : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: "#475569" }}>Longitude</span>
                  <span className="text-sm font-mono font-semibold" style={{ color: "#e2e8f0" }}>
                    {typeof laporan.lng === "number" ? laporan.lng.toFixed(6) : "—"}
                  </span>
                </div>
              </div>
              <a
            ...
            {/* Metadata */}
            <div className="rounded-xl p-4 space-y-3" style={{ background: "#0f172a", border: "1px solid #1e293b" }}>
              <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "#64748b" }}>Metadata</p>
              <div className="space-y-1.5">
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: "#475569" }}>ID Laporan</span>
                  <span className="text-xs font-mono" style={{ color: "#64748b" }}>
                    {laporan.id ? laporan.id.slice(0, 16) + "…" : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: "#475569" }}>ID Pengguna</span>
                  <span className="text-xs font-mono" style={{ color: "#64748b" }}>
                    {laporan.user_id ? laporan.user_id.slice(0, 16) + "…" : "Anonim"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm" style={{ color: "#475569" }}>Status Vaksin</span>
                  <span className="text-sm font-semibold"
                    style={{ color: laporan.sudah_vaksin === true ? "#86efac" : laporan.sudah_vaksin === false ? "#fca5a5" : "#475569" }}>
          ...
          {/* Aksi */}
          <div className="flex justify-end gap-3 pt-2" style={{ borderTop: "1px solid #1e293b" }}>
            <Btn onClick={onTutup} variant="ghost">Tutup</Btn>
...
function TabLaporan() {
  ...
  const hapus = async (id) => {
    if (!confirm("Yakin hapus laporan ini? Tindakan ini tidak bisa dibatalkan.")) return;
    try {
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3 items-center flex-wrap">
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl"
          ...
          <select value={jam} onChange={e => { setJam(e.target.value); setHalaman(1); }}
            className="text-sm outline-none bg-transparent"
            style={{ color: "#e2e8f0" }}>
            <option value="" style={{ background: "#111827" }}>Semua Waktu</option>
            <option value="24" style={{ background: "#111827" }}>24 Jam Terakhir</option>
            <option value="48" style={{ background: "#111827" }}>48 Jam Terakhir</option>
            <option value="168" style={{ background: "#111827" }}>7 Hari Terakhir</option>
          </select>
        </div>
        <Btn onClick={muat} variant="ghost">Muat Ulang</Btn>
        {data && (
      ...
      <div className="rounded-2xl overflow-hidden"
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: "linear-gradient(90deg,#0f172a,#0d1627)" }}>
              {["Wilayah", "Gejala Utama", "Skor", "Keparahan", "Kelompok Usia", "Waktu", "Aksi"].map(h => (
                <th key={h} className="px-5 py-4 text-left text-xs font-bold uppercase tracking-wider"
                  style={{ color: "#64748b" }}>
                  {h}
                </th>
            ...
            {loading ? (
              ...
              return (
                <tr key={r.id}
                  ...
                  onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? "#080d1a" : "#0a1020"}
                >
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-1">
                      {r.gejala?.slice(0, 2).map(g => (
                        <span key={g} className="px-2 py-0.5 rounded text-xs font-medium"
                          style={{ background: "#1e293b", color: "#cbd5e1" }}>
                          {g.replace(/_/g, " ")}
                        </span>
                      ))}
                      {r.gejala?.length > 2 && (
                        <span className="px-2 py-0.5 rounded text-xs" style={{ color: "#475569" }}>
                          +{r.gejala.length - 2}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-col gap-1">
                      <span className="text-base font-bold" style={{ color: warna.text }}>
                        {r.skor_influenza}
                      </span>
                      ...
                      <div className="flex gap-0.5">
                        {Array.from({ length: 10 }).map((_, idx) => (
                          <div key={idx} className="w-1.5 h-3 rounded-sm"
                            style={{ background: idx < r.tingkat_keparahan ? warna.text : "#1e293b" }} />
                        ))}
                      </div>
                      <span className="text-xs font-bold ml-1" style={{ color: "#94a3b8" }}>
...
export default function AdminApp() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4"
        ...
        <div className="w-10 h-10 rounded-full border-2 border-t-blue-500 animate-spin"
          style={{ borderColor: "#1e293b", borderTopColor: "#3b82f6" }} />
        <p className="text-sm" style={{ color: "#475569" }}>Memuat panel admin…</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: "#060b18", color: "#f1f5f9" }}>
      ...
      {/* ── Navbar ── */}
      <header style={{ background: "#0a1020", borderBottom: "1px solid #1e293b", position: "sticky", top: 0, zIndex: 50 }}>
        <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs leading-tight" style={{ color: "#475569" }}>Panel Administrator</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl"
              ...
              <span className="text-sm font-medium" style={{ color: "#cbd5e1" }}>{adminAktif.username}</span>
              <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "#1e3a5f", color: "#93c5fd" }}>
                Admin
              </span>
            </div>
            <a href="/"
            ...
            <button
              onClick={() => { logout(); setLocalAdmin(null); }}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition"
